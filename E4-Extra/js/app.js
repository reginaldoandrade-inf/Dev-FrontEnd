...
import { renderizarQuadro, atualizarStatusAcessivel, exibirModalComanda, fecharModalComanda } from './renderizacao.js';
...
function obterTarefasFiltradas(estadoAtual) {
  ...
  if (estadoAtual.busca.trim() !== '') {
    ...
    resultado = resultado.filter((t) => 
      normalizarTexto(t.titulo).includes(termo) ||
      normalizarTexto(t.projeto).includes(termo) ||
      normalizarTexto(t.responsavel).includes(termo)
    );
  }
...
function atualizarInterface() {
  ...
  if (visiveis === 0) {
    ...
    atualizarStatusAcessivel(`Exibindo ${visiveis} de ${total} pedido(s).`);
  }
...
function inicializarDragAndDrop() {
  ...
  quadro.addEventListener('dragstart', (e) => {
    const cartao = e.target.closest('article.card-pizza-item');
    if (!cartao) return;

    estado.idItemArrastado = cartao.dataset.id;
    cartao.classList.add('dragging');
  });

  quadro.addEventListener('dragend', (e) => {
    const cartao = e.target.closest('article.card-pizza-item');
    if (cartao) cartao.classList.remove('dragging');

    document.querySelectorAll('[data-lista-status]').forEach((ul) => {
  ...
  quadro.addEventListener('dragover', (e) => {
    e.preventDefault();
    const lista = e.target.closest('[data-lista-status]');
    if (lista) lista.classList.add('drag-over');
  });

  quadro.addEventListener('dragleave', (e) => {
    const lista = e.target.closest('[data-lista-status]');
    if (lista) lista.classList.remove('drag-over');
  });

  quadro.addEventListener('drop', (e) => {
    e.preventDefault();
    const lista = e.target.closest('[data-lista-status]');
    if (!lista || !estado.idItemArrastado) return;

    const novoStatus = lista.dataset.listaStatus;
    const tarefa = estado.tarefas.find((t) => String(t.id) === String(estado.idItemArrastado));
...
function inicializarEventos() {
  ...
  const btnAvancarModal = document.getElementById('btn-avancar-estacao');
  ...
  quadro?.addEventListener('click', (e) => {
    ...
    if (tarefa) {
      estado.tarefaModalId = tarefa.id;
      exibirModalComanda(tarefa);
    }
  });

  btnAvancarModal?.addEventListener('click', () => {
    if (!estado.tarefaModalId) return;

    const tarefa = estado.tarefas.find((t) => String(t.id) === String(estado.tarefaModalId));
    if (tarefa) {
      ...
      if (idxAtual < ordem.length - 1) {
        tarefa.status = ordem[idxAtual + 1];
        fecharModalComanda();
        atualizarInterface();
      }
    }
  ...
  modalOverlay?.addEventListener('click', (e) => {
    if (e.target === modalOverlay) fecharModalComanda();
  });
